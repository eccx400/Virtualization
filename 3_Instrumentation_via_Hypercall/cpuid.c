...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	exit_number = ecx = kvm_rcx_read(vcpu);
	...
	} else if (eax == 0x4FFFFFFE){ /*CMPE 283 Assignment 3*/
		pr_info("CMPE 283 Leaf function for eax == 0x4FFFFFFE");
		pr_info("Exit number in ecx = %u", exit_number);

		if (exit_number < 0 || exit_number >= 69 || exit_number == 35 || exit_number == 38 || exit_number == 42 || exit_number == 65) {
			eax = 0;
			ebx = 0;
			ecx = 0;
			edx = 0xFFFFFFF;
